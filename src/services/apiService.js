...
apiClient.interceptors.request.use(
  async (config) => {
    const betterAuthToken = await AsyncStorage.getItem('better-auth.session_token');
    const token = betterAuthToken || await AsyncStorage.getItem('accessToken');
    if (token) {
    ...
    if (config.data && typeof config.data === 'object' && !(config.data instanceof FormData)) {
      ...
      if (url.includes('/auth/login') || url.includes('/auth/register')) {
        ...
        if (publicKeyPem) {
          const { encryptedPayload, encryptedAesKey, aesKey } = await encryptHybrid(
            config.data,
            publicKeyPem
          );
...
apiClient.interceptors.response.use(
  async (response) => {
    ...
    if (response.data && typeof response.data === 'object' && response.data.encryptedPayload) {
      try {
        if (aesKey) {
          response.data = decryptWithKey(response.data.encryptedPayload, aesKey);
        } else {
  ...
  async (error) => {
    ...
    if (error.response?.data && typeof error.response.data === 'object' && error.response.data.encryptedPayload) {
      try {
    ...
    const isDecryptionError =
      (error.response?.status === 400 && errMsg.includes('encrypted payload')) ||
